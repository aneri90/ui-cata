...
function ZGV:OnEnable()  --PLAYER_LOGIN
	...
	self:AddEventHandler("UPDATE_MOUSEOVER_UNIT",ZGV.HandleMouseover)
...
function ZGV:Startup_LoadGuides_Threaded()
	...
	if #self.registeredguides>0 then
		...
		ZGV.IsPandariaRemix =  PlayerGetTimerunningSeasonID and PlayerGetTimerunningSeasonID() == Constants.TimerunningConsts.TIMERUNNING_SEASON_PANDARIA
		-- yes, we check again, since PlayerGetTimerunningSeasonID does not return values at first frame or first load
		if ZGV.IsRetail then
			-- remove guides not matching current remix state
			for gi=#ZGV.registeredguides,1,-1 do
				local guide = ZGV.registeredguides[gi]
				if ZGV.IsPandariaRemix~=(not not guide.headerdata.pandariaremix) then
					table.remove(ZGV.registeredguides,gi)
					ZGV.RegisteredGuidesTitles[guide.title]=nil
				end
			end
		end
...
function ZGV.HandleMouseover()
	-- nothing for us to do
	if not ZGV.db.profile.mousemarkers then return end
	...
	local target = ZGV.GetUnitId("mouseover")
	local marker = 0

	for _,goal in ipairs(ZGV.CurrentStep.goals) do
		if goal.targetid==target and goal.action=="kill" then
			marker = 8
		end
		if goal.npcid==target and goal.action=="talk" then
			marker = 1
		end
	end
	...
	if GetRaidTargetIndex("mouseover")~=marker then

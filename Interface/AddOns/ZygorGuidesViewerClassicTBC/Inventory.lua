...
function Inventory:FindItemsToBuy()
	...
	if playerMoney >= totalCost then
		...
		for name,item in pairs(self.ItemsToBuy) do itemtext = itemtext..item.name.." x |cffff0000"..item.amount.."|r|n" end
		local text = L['loot_autobuyframetext']:format(itemtext,GetMoneyString(totalCost))
		ZGV.NotificationCenter:AddEntry("goldbuy",title,text,{transient=true})
...
function Inventory.OnEvent(self, event)
	...
	elseif event=="MERCHANT_CLOSED" then
		...
		ZGV.NotificationCenter:RemoveEntriesByType("goldbuy")
	...
	elseif event=="ZGV_STEP_FINALISED" then
		Inventory:FindItemsToBuy() 
	end
end

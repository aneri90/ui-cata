...
do
	...
	if Lib then
			...
			for i,port in ipairs(Lib.data.portkeys) do repeat
				...
				if reject_reason then
					tinsert(Lib.debug_portkeys,{tostring=function() return ("%s #%d - |cffff0000%s|r")
						:format(port.item and C_Item.GetItemInfo(port.item) or GetSpellInfo(port.spell) or "name not ready",port.item or port.spell, reject_reason or "?") end,
						data=port
					})
					break  --next item
				end
				...
				tinsert(Lib.debug_portkeys,{tostring=function() return ("%s #%d - cost:|cffffff44%.1f|rs, cd:|cff88ff00%.1f|rs, item cd:|cff00ff88%.1f|rs, final cost:|cff88aaff%.1f|rs")
					:format(port.item and C_Item.GetItemInfo(port.item) or GetSpellInfo(port.spell) or "name no ready",port.item or port.spell,
		...
		function Lib:IsDestinationImpossible(mymap,destmap)
			...
			elseif cont~=424 and Lib.IsPandariaRemix then
				return true,"REMIX_LOCKED","You can't get leave Pandaria on this character."
			...
			for neigh,neighlink in current:IterNeighs() do
				...
				then
					...
					-- Promote comfortable modes of travel.
						if mode=="taxi" and Lib:GetCFG("prefer_taxi") then
					...
					if (mode=="walk" or mode=="fly") and current.parentlink and current.parentlink.mode=="taxi" and current:IsTaxiKnown()==false then -- walking from an unknown taxi, means we have LANDED on it. Penalize!
					...
					if neigh.type=="taxi" and (mode=="walk" or mode=="fly") then -- departing from conditioned taxis is bad
						...
						elseif known==nil and couldbe then
							mycost=mycost+TAXI_FROWN_UNKNOWNS -- and discurage maybes
						end

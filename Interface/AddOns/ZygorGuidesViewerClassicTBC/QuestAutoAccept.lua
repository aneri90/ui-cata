...
function ZGV:QuestAutoAccept_InGossip()
	...
	for qnum,questInfo in ipairs(quests) do
		...
		if self.CurrentStep then
			for i,goal in ipairs(self.CurrentStep.goals) do
				if ((goal.action=="accept" and goal.quest and goal.questid==questInfo.questID and goal:GetStatus()=="incomplete") or (goal.autoacceptany and goal.autoacceptany[questInfo.questID])) and not goal.noautoaccept then
...
function ZGV:QuestAutoAccept_InDetail()
	...
	if self.CurrentStep then
		for i,goal in ipairs(self.CurrentStep.goals) do
			if ((goal.action=="accept" and goal.quest and goal.quest.id==id and goal:IsCompleteable()) or (goal.autoacceptany and goal.autoacceptany[id])) and not goal:IsObsolete() and not goal.noautoaccept then
...
function ZGV:QuestAutoTurnin_InGossip()
	...
	for qnum, questInfo in ipairs(quests) do
		...
		if self.CurrentStep then
			for i,goal in ipairs(self.CurrentStep.goals) do
				if (goal.action=="turnin" and goal.questid == questInfo.questID and goal:GetStatus()=="incomplete") or (goal.autoturninany and goal.autoturninany[questInfo.questID] and questInfo.isComplete) then
...
function ZGV:QUEST_COMPLETE()
	...
	if self.db.profile.questitemselector and GetNumQuestChoices() > 1 then
		...
		if itemindex then
			questitems:ShowQuestRewardGlow(itemindex,reason)
		end
	end

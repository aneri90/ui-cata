...
function ZGV:Options_DefineOptionTables()
	...
	do
		...
		AddOption('',{ type = 'description', name=L['opt_nc_types'], hidden=function() return not self.db.profile.nc_showall end, font=ZGV.font_dialog})
			...
			AddOption('nc_gear_max',{type = 'toggle', width = "full", _default = false, indent=20, hidden=function() return not self.db.profile.nc_showall end, disabled = function() return not self.db.profile.nc_enable or not self.db.profile.nc_gear or not self.db.profile.autogear end,})
	...
	do
		...
		AddOption('mousemarkers',{ type = 'toggle', width = "full", _default=false})

...
function ZGV_Notification_Entry_Template_Mixin:UpdateMode() 
	if ZGV.db.profile.nc_size==1 or self.entry.text=="" then -- compact
		self.text:Hide()
		if self.single then
...
function ZGV_Notification_Entry_Template_Mixin:OnLeave()
	if self.single and self.entry then self:StartFadeTimer()  end
	self.close:Hide()
	GameTooltip:Hide()
end
...
function ZGV_Notification_Entry_Template_Mixin:UpdateHeight() 
	...
	if self.single then height = max(height,25) end
...
function NC:Startup()
	...
	NC.EntrySettings.viewer:SetScript("OnClick",function() ZGV:ToggleFrame() end)
	NC.EntrySettings.viewer.tooltip = "Show / hide viewer"
	NC.EntrySettings.settings:SetScript("OnClick",function() NC:ShowSub() end)
	
	NC.EntrySettings.menuList = {
		{
			func = function() NC:ClearNotifications() end,
			text = L["nc_reset"],
			notCheckable = 1
		},
		{
			func = function() ZygorGuidesViewer:OpenOptions() end,
			text = L["nc_settings"],
			notCheckable = 1
		},
	}
...
function NC:AddEntry(notiftype,title,text,data)
	...
	local ident = data.ident or debugprofilestop()..notiftype
	entry.ident = ident
	entry.func = function() 
...
function NC:RemoveEntry(ident)
	...
	if NC.SingleNotif.entry and NC.SingleNotif.entry.ident == ident then
...
function NC:ApplySkin()
	ZGV.IconSets.OptionsIcons.display:AssignToButton(NC.EntrySettings.viewer)
	NC.EntrySettings.title:SetFont(FONTBOLD,14)
	NC.EntrySettings.title:SetTextColor(1,1,1,1)
	NC.EntrySettings.settings:ApplySkin()
	NC.EntrySettings.viewer:ApplySkin()
end
...
function NC:CheckDynamicNotifications()
	...
	if welcome then
		...
		if ZGV.db.profile.welcomehash ~= welcomehash and ZGV.db.profile.nc_welcome then 
			NC:AddEntry("welcome",welcome.title,format_text(welcome.text),{cleartype=true,keeponclick=true})
	...
	if guides then
		...
		if ZGV.db.profile.guideshash ~= guideshash and ZGV.db.profile.nc_general then 
			NC:AddEntry("guides",guides.title,format_text(guides.text),{cleartype=true,keeponclick=true})
	...
	if features then
		...
		if ZGV.db.profile.featureshash ~= featureshash and ZGV.db.profile.nc_general then 
			NC:AddEntry("features",features.title,format_text(features.text),{cleartype=true,keeponclick=true})

...
local GetTime, GetInstanceInfo, GetSpellInfo = GetTime, GetInstanceInfo, GetSpellInfo
...
local SendWA_Event = WeakAuras.ScanEvents
...
CORE.TargettedCast = function(sourceGUID, e)
    ...
    if unitName then
        ...
        SendWA_Event(e, raidId, unitName)
    end
end
...
CORE.IsDifficulty = function(...)
    local difficulty = CORE.difficulty or GetInstanceInfo()
    for i = 1, select("#", ...) do
...
CORE.Schedule = function(ENTRY, mechanic, after, func, ...)
    if ENTRY.schedulers[mechanic] then WeakAuras.timer:CancelTimer(ENTRY.schedulers[mechanic]) end
...
CORE.IsStage = function(ENTRY, n)
    if not ENTRY.stage then return true end
...
local Handlers = {
    ...
    CANCEL = function(a, caller, mechanic, cancelType)
...
local Prototype = {
    ...
    CANCEL = function(ENTRY, mechanic, cancelType)
        ...
        SendEvent(Handlers.CANCEL, mechanic, cancelType)
    end,
    ...
    PRE_WARNING = function(ENTRY, mechanic, showIn, duration, counter)
        local spellName, icon = GetMechanicInfo(ENTRY, mechanic)
        if ENTRY.schedulers[mechanic] then WeakAuras.timer:CancelTimer(ENTRY.schedulers[mechanic]) end
        ENTRY.schedulers[mechanic] = WeakAuras.timer:ScheduleTimer(SendEvent, showIn, Handlers.PRE_WARNING, mechanic, duration or 5, spellName, icon, counter or 0)
    end,
    ...
    COOLDOWN = function(ENTRY, mechanic, duration, sourceName, forceHide, count)
        if not CORE.GetSettings('cooldown', 'visibility') then return end
        local spellName, icon = GetMechanicInfo(ENTRY, mechanic)
...
CORE.SetDifficulty = function()
    CORE.difficulty = GetInstanceInfo()
end
...
Dungeon:SetScript("OnEvent", function(self, event, ...)
	if event == "PLAYER_ENTERING_WORLD" then
		C_Timer.After(3, function() CORE.SetDifficulty() end)
	end
end)